...
module pancake::swap {
    ...
    */
    ...
    spec withdraw_fee_noauth {
        ...
        // Replace coin::is_account_registered with coin::CoinStore existence check
        aborts_if !exists<coin::CoinStore<LPToken<X, Y>>>(swap_info.fee_to);
        aborts_if !exists<TokenPairMetadata<X, Y>>(RESOURCE_ACCOUNT);

        aborts_if !exists<coin::CoinStore<LPToken<Y, X>>>(swap_info.fee_to);
        aborts_if !exists<TokenPairMetadata<Y, X>>(RESOURCE_ACCOUNT);

        let metadata1 = global<TokenPairMetadata<X, Y>>(RESOURCE_ACCOUNT);
        aborts_if coin::value(metadata1.fee_amount) == 0;

        let metadata2 = global<TokenPairMetadata<Y, X>>(RESOURCE_ACCOUNT);
